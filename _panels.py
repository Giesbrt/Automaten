...
from PySide6.QtWidgets import (QWidget, QListWidget, QStackedLayout, QFrame, QSpacerItem, QSizePolicy, QLabel,
                               QFormLayout, QLineEdit, QListWidgetItem,
...
class UserPanel(Panel):
    ...
    def __init__(self, automaton_type: str, parent: QWidget | None = None) -> None:
        ...
        # Connect signals
        self.side_menu_animation.valueChanged.connect(self.side_menu_animation_value_changed)
        self.menu_button.clicked.connect(self.toggle_side_menu)

        # Integrate search guide and item list into the side menu
        self.search_guide()
        self.items_list = QListWidget(self)
        self.populate_items_list()
        self.side_menu_layout.addWidget(self.items_list)

        # Initialize token list (example)
        self.token_list = []

    # Methods for setting and getting token list
    ...
    # Method for adding a search guide to the side menu
    def search_guide(self):
        """Add search field to side menu"""
        search_label = QLabel("Suchen:", self.side_menu)
        search_field = QLineEdit(self.side_menu)
        search_field.textChanged.connect(self.search_items)

        self.side_menu_layout.addWidget(search_label)
        self.side_menu_layout.addWidget(search_field)
    ...
    def populate_items_list(self):
        ...
        example_items = [
            ...
            {"title": "Schriftart ändern", "description": "Gehe in die settings (Oben rechts) und klcike auf Design. Wähle eine beliebige Schriftart aus"},       
        ]

        for item_data in example_items:
            self.add_item_to_list(item_data["title"], item_data["description"])

    def add_item_to_list(self, title, description):
        """Fügt einen Eintrag mit Button zur Liste hinzu und sorgt für korrektes Einpassen des Textes"""
        widget = QWidget()
        layout = QVBoxLayout(widget)

        button = QPushButton(f"{title} ▼", self.side_menu)
    ...
    def resizeEvent(self, event):
        ...
        # Adjust side menu position
        if self.side_menu.x() < 0:
            self.side_menu.setGeometry(-width, 0, width, height)
            self.menu_button.move(width + 40, 20)
        else:
            self.side_menu.setGeometry(0, 0, width, height)
            self.menu_button.move(40, 20)

        # Dynamic customization of buttons and labels in the list
        for i in range(self.items_list.count()):
            item = self.items_list.item(i)
            widget = self.items_list.itemWidget(item)
            if widget:
                widget.setMaximumWidth(self.side_menu.width() - 10)
                button = widget.findChild(QPushButton)
                if button:
                    button.setMaximumWidth(self.side_menu.width() - 10)
                    button.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
                label = widget.findChild(QLabel)
                if label:
                    label.setMaximumWidth(self.side_menu.width() - 10)

        # State Menu Adjust position
        ...
        # Control Menu Position anpassen
        if self.state_menu.visible:
