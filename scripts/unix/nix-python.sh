...
if [ -d "$flake_root/unix" ]; then
  flake_root="$flake_root/unix"
fi
...
project_root="$(dirname "$(dirname "$flake_root")")"  # Because for me this is in ./scripts/unix
...
#cat > "$project_root/.nixpy/bin/python" <<EOF
##!/bin/sh
#. "$project_root/.nixpy/.venv/bin/activate"
#exec nix --extra-experimental-features 'nix-command flakes' develop "$flake_root" --command python "\$@"
#EOF
cat > "$project_root/.nixpy/bin/python" <<EOF
#!/bin/sh
exec nix --extra-experimental-features 'nix-command flakes' develop "$flake_root" --command \
    sh -c '. "$project_root/.nixpy/.venv/bin/activate" && exec python "\$@"' _ "\$@"
EOF
