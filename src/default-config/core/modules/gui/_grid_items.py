...
class StateItem(QGraphicsItemGroup):
    ...
    def itemChange(self, change, value):
        ...
        if change == QGraphicsItem.GraphicsItemChange.ItemSelectedChange:
            if hasattr(self, 'is_selected') and self.is_selected == value:
...
class TransitionItem(QGraphicsLineItem):
    ...
    def set_transition_function(self, transition_function: 'TransitionFunction') -> None:
        """Sets the transition function for the transition.

        :param transition_function: The transition function object.
        """
        self.transition_function = transition_function

    def get_transition_function(self) -> 'TransitionFunction':
        """Retrieves the transition function associated with the transition.

        :return: The transition function object.
        """
    ...
    def set_condition(self, condition: _ty.List[str]):
        ...
        """
        self.transition_function.set_condition(condition)
    ...
    def set_token_lists(self, token_lists: _ty.Tuple[_ty.List[str], _ty.List[str]]):
        ...
        """
        self.transition_function.update_token_list(token_lists)
    ...
    def update_position(self) -> None:
        ...
        if self.transition_function:
            ...
            button_frame = self.transition_function.token_button_frame
            token_list_frame = self.transition_function.token_list_frame

            parent_center = button_frame.parentItem().mapFromScene(center)

            button_x = parent_center.x() - button_frame.size().width() / 2
            button_y = parent_center.y() - button_frame.size().height() / 2

            button_frame.setPos(button_x, button_y)

            gap = 5
            token_list_x = button_x
            token_list_y = button_y + button_frame.size().height() + gap
            token_list_frame.setPos(token_list_x, token_list_y)
    ...
    def mousePressEvent(self, event):
        """Handles mouse press events on the transition."""
        print('TransitionItem mousePressEvent')
